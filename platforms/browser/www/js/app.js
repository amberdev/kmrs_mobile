...
document.addEventListener("pageinit", function (e) {
    ...
    switch (e.target.id)
    {
        ...
        case "page-paymentoption":
            ...
            // callAjax("listClientCards", "client_token=" + getStorage('client_token'));
            k=1;
        ...
        beforeSend: function () {
            if (ajax_request != null) {
                ...
                timer = setTimeout(function () {
                    hideAllModal();
                    ajax_request.abort();
                 toastMsg(getTrans('The request is taking longer than expected. Please try again.', 'request_time'));
                   }, 40000);
                switch (action)
                {
                    case "registerMobile":
        ...
        success: function (data) {
            ...
            if (data.code == 1) {
             ...
             //           onsenAlert(getTrans("Done"));
                    ...
                    case "listClientCards":
                        console.log(data.details.giftcards);
                        $('.cardid').children('option:not(:first)').remove();
                        for(j=0;j<data.details.giftcards.length;j++)
                        {
                            console.log(data.details.giftcards[j]);
                            $('.cardid').append($('<option>', {
                                value: data.details.giftcards[j],
                                text: data.details.giftcards[j]
                            }));
                        }
                    break;

                    case "updateCheckoutOfflinegiftcard":
...
//                                sNavigator.pushPage("atzPaymentForm.html", options);
       ...
       //                    loader.hide();
                    ...
                    case "applyGiftCardAmount":
                        dump(data.details);
                        $(".giftamount_amount").val(data.details.amount);
                        // $(".voucher_type").val(data.details.voucher_type);
                        // $(".apply-voucher").hide();
                    ...
                    case "getMerchantCClist":
                        ...
                        break;
                    case "getMerchantCClistatz":
                        fillCCListatz(data.details);
                        break;
                    case "saveCreditCard":
                    case "deleteCreditCard":
                        ...
                        break;
                    case "saveCreditCardatz":
                    case "deleteCreditCardatz":
                        sNavigator.popPage({cancelIfRunning: true});
                        var params = "merchant_id=" + getStorage("merchant_id");
                        params += "&client_token=" + getStorage("client_token");
                        callAjax("getMerchantCClistatz", params);
                        break;
                    case "loadCC":
                    ...
                    case "loadCCatz":
                        $("#savebtnatz").hide();
                        $(".card_name").val(data.details.card_name);
                        $('.cc_number').get(0).type = 'text';
                        $(".cc_number").val("XXXXXXXX" + data.details.credit_card_number.substr(data.details.credit_card_number.length - 4));
                        $(".cvv").hide();
                        $("#month_expiry_atz").hide();
                        $("#year_expiry_atz").hide();
                        //      $(".cvv").val(data.details.cvv);
                        //    $(".billing_address").val(data.details.billing_address);
                        //                       $(".expiration_month").val(data.details.expiration_month);
                        //                   $(".expiration_yr").val(data.details.expiration_yr);
                    ...
                    if(data.details.status == false){
                            ...
                            $("#switchbutton").show();
                            var tmp_data = "";
                            if (getStorage("category_type") == "catering") {
                                tmp_data = "regular";
                            } else {
                                tmp_data = "catering";
                             ...
                             setTimeout(function(){ 
                  onsenAlert("Toggle the switch at the top to view " + tmp_data + " items");
 }, 2000);
          
                        }
                        break;
//                    case "itemcateringornot":
//
//                    if(data.details.status == true && getStorage("category_type") == "catering"){
//                                toastMsg("Case1");
//                          setStorage("proceed_further",true);     
//                    }else{
//                        if(data.details.status == false && getStorage("category_type") == "regular"){
//                            toastMsg("Case2");
//                                setStorage("proceed_further",true);     
//                        }
//                        else{
//                            toastMsg("Case3");
//                          setStorage("proceed_further",false);     
//                        }
//                    }
//                        break;
                    case "getItemCount":
...
{
    toastMsg("This menu is currently password protected as part of the FoodSquire internal review process.");
}
...
function displayRestaurantResults(data, target_id)
{
    ...
    $.each(data, function (key, val) {
        ...
        //dump(val);
 if(val.password_protected=='yes')
        {
            htm += '<ons-list-item modifier="tappable" class="list-item-container" onclick="password_protected();" >';
        }
        else
        {
        ...
        if ((val.catering)) {
                htm += '<p class="center">' + "Catering" + ' <i class="green-color ion-android-checkmark-circle"></i></p>';
        ...
        if(val.fs_can_deliver == 1){
        htm += '<div class="logo-wrap2" >';
        htm += '<div class="img_loaded" >';
        htm += '<img src= css/images/inside.png />';
        htm += '</div>';
        htm += '</div>';
        
        }
        if(val.fs_can_deliver == 0){
        htm += '<div class="logo-wrap2" >';
        htm += '<div class="img_loaded" >';
        htm += '<img src= css/images/outside.png />';
        htm += '</div>';
        htm += '</div>';
        }
    }
...
function displayItem(data)
{
    ...
    if (!empty(data.addon_item)) {
     ...
     htm += cartFooter(data.currency_code,data.is_omnivore_setup);
...
jQuery(document).ready(function () {
    ...
    $(document).on({
        ...
        // $('#char_limit_fl').keypress(function(e) {
    $(document).on("keypress", "#char_limit_fl", function (e) {
        var tval = $('#char_limit_fl').val(),
            tlength = tval.length,
            set = 20,
            remain = parseInt(set - tlength);
         
        if (remain <= 0 && e.which !== 0 && e.charCode !== 0) {
            $('#char_limit_fl').val((tval).substring(0, tlength - 1));
            return false;
        }
    });
    ...
    $(document).on("click", ".transaction_type", function () {
        ...
        callAjax("loadCart", "merchant_id=" + getStorage('merchant_id') + "&search_address=" +
                ...
                getStorage("transaction_type") + extra_params + "&device_id=" + getStorage("device_id") + "&category_type=" + getStorage("category_type"));
    });
    $(document).on("click", ".payment_list", function () {
        ...
        switch ($(this).val())
        {
            ...
            case "atz":
                ...
                sNavigator.pushPage("cclistatz.html", options);
                break;
            default:
                if (paypal_card_fee > 0) {
                    $(".total-amount").html(getStorage("order_total"));
...
//        callAjax("itemcateringornot", "item_id=" + item_id);
//        toastMsg(getStorage("proceed_further"));    
//        if(getStorage("proceed_further") == false){
//            onsenAlert("Please choose items of "+getStorage("category_type"));
//            return;
//        }
        cart[cart.length] = {
            "item_id": item_id,
...
function showCart()
{
    ...
    var options = {
        ...
        onTransitionEnd: function () {
            ...
            params += "&category_type=" + getStorage("category_type");
...
function displayCart(data)
{
    ...
    if(!empty(data.cart)){
        if(getStorage("category_type") == "catering"){
           if(transaction_type == "delivery"){
            htm += tplCartRow('<b class="trn" >Min sub-total amount for Delivery</b>', data.minorder.minorderarray.delivery);
           }
           if(transaction_type == "pickup"){
            htm += tplCartRow('<b class="trn">Min sub-total amount for Pickup</b>', data.minorder.minorderarray.pickup);
           }
    ...
    $('input[type="checkbox"].delivery_asap').click(function(){
    ...
    $(".transaction_type").click(function(){
        $(".delivery_time").removeAttr("disabled"); 
        $('input[type="checkbox"].delivery_asap').prop('checked', false);
    });
...
function applyCartChanges()
{
    ...
    if (!empty($(".item-qty"))) {
        ...
        callAjax("loadCart", "merchant_id=" + getStorage('merchant_id') + "&search_address=" +
                ...
                getStorage("transaction_type") + extra_params + "&device_id=" + getStorage("device_id") + "&tips_percentage=" + getStorage("tips_percentage") + "&category_type=" + getStorage("category_type"));
    }
}
...
//ASAP Will not work when restaturant is closed
    ...
    extra_params += "&category_type=" + getStorage("category_type");
...
function placeOrder()
{
    if ($('.payment_list:checked').length > 0) {
        ...
        if (selected_payment == "atz") {
            if (empty(getStorage("cc_id_atz"))) {
                $("#placeorderbtn").prop('disabled', false);
             //      loader.hide();
...
function purchaseGiftCards()
{
    if (isLogin()) {
        menu.setMainPage('purchaseGiftCards.html', {closeMenu: true});
    } else {
        menu.setMainPage('prelogin.html', {closeMenu: true})
    }
}
...
function listGiftCard()
{
    ...
    {   
        var params = "client_token=" + getStorage('client_token');
        callAjax("listClientCards", params);
    }
    else
    {
        onsenAlert(getTrans('No gift card added', 'invalid_gift_card_added'));
    }
}

function applyGiftCardAmount()
{
    amnt=$(".total-amount").text().split("$");
    // alert(amnt[1]);
    redeem_gift_amount = $(".redeem_gift_amount").val();
    cardid=$(".cardid").val();
    if (redeem_gift_amount != "") {
        var params = "giftcardamount=" + redeem_gift_amount;
        params += "&client_token=" + getStorage("client_token");
        params += "&cart_sub_total=" + amnt[1];
        ...
        // }
        // params += "&tips_percentage=" + getStorage("tips_percentage");
        callAjax("applyGiftCardAmount", params);

    } else {
        onsenAlert(getTrans('Please enter amount you would like to redeem using gift card.', 'invalid_gift_amount'));
    }
}

function paywithcard()
{
    firstname=$(".gift_card_first_name").val();
    email=$(".gift_card_email").val();
    phone=$(".gift_card_phone").val();
    gift_value=$(".gift_card_value").val();

    if(firstname!='' && email!='' && phone!='' && gift_value!='')
    {
        var params = "giftcardamount=" + redeem_gift_amount;
        params += "&client_token=" + getStorage("client_token");
        params += "&cart_sub_total=" + amnt[1];
        ...
        callAjax("PurchaseGiftCard", params);
    }
    else
    {
        onsenAlert(getTrans('Please enter all the details', 'invalid_gift_details'));
    }

}

function updateCheckoutOfflinegiftcard()
{
    client_id = $(".card_id").val();
    if (client_id != "") {
        // var params = "giftcardamount=" + redeem_gift_amount;
        var params = "&client_token=" + getStorage("client_token");
        params += "&card_id=" + client_id;
        // params += "&cardid=" + getStorage("cart_sub_total");
        // transaction_type = getStorage("transaction_type");
        // params += "&transaction_type=" + getStorage("transaction_type");
        /*if ( transaction_type=="delivery"){
         params+="&cart_delivery_charges="+ getStorage("cart_delivery_charges");
        ...
        // }
        // params += "&tips_percentage=" + getStorage("tips_percentage");
        callAjax("updateCheckoutOfflinegiftcard", params);
        // listGiftCard();
    } else {
        onsenAlert(getTrans('Please enter a gift card ID before submitting', 'invalid_gift_card'));
    }
}
...
function checkBalance()
{
    ...
    if (client_id != "") {
        ...
        callAjax("ClientCardBalance", params);
        // listGiftCard();
    } else {
        onsenAlert(getTrans('Please select card id.', 'invalid_gift_card'));
    }
}
...
function atzPayNow(params)
{


    params += "&cc_id=" + getStorage('cc_id_atz');
    params += "&cc_number=" + getStorage('cc_number_atz');
    params += "&expiration_month=" + getStorage('expiration_month_atz');
    params += "&expiration_yr=" + getStorage('expiration_yr_atz');
    params += "&cvv=" + getStorage('cvv_atz');


    params += "&order_id=" + getStorage('order_id');
    params += "&currency_code=" + getStorage('currency_code');
    params += "&paymet_desc=" + getStorage('paymet_desc');
    params += "&total_w_tax=" + getStorage('total_w_tax');
...
{
    callAjax("getcateringstatus", "mtid=" + getStorage("merchant_id"));
 }
function switchcategory()
{
    var class_name = "amcatering";
    var other_class_name = "nocatering";
    if (getStorage("category_type") == "regular") {
       // toastMsg("Case1");

        removeStorage("category_type");
        setStorage("category_type", "catering");
        class_name = "nocatering";
        other_class_name = "amcatering";
        ...
        if (getStorage("category_type") == "catering") {
         //   toastMsg("Case2");
            ...
            removeStorage("category_type");
            setStorage("category_type", "regular");
            class_name = "amcatering";
            other_class_name = "nocatering";
                    $(".delivery_asap_wrap").show();

        } else {

            if (getStorage("category_type") == "") {
           //     toastMsg("Case3");
...
//    toastMsg(getStorage("category_type"));
 setTimeout(function(){ 
if($("#switchbutton").is(":visible")){
    toastMsg("Now viewing "+getStorage("category_type")+ " items");
} 
 }, 2000);

    $("." + class_name).each(function() {
    $(this).hide();
    $(this).parentsUntil( "#lazy-parent" ).hide();
//    $(this).parent( "#lazy-parent" ).hide();
    
    });
    
//    $("." + class_name).hide();
//    $("." + other_class_name).show();
    
    $("." + other_class_name).each(function() {
    $(this).show();
    $(this).parentsUntil( "#lazy-parent" ).show();
//    $(this).parent( "ons-list-item" ).show();
    
    });
//    if(class_name == "amcatering"){
//        $("#switchbutton").html("Catering Items");
//    }else{
//        $("#switchbutton").html("Regular Items");
//        
//    }
...
function useThisLocation()
{
    ...
    switch (map_address_action) {
        ...
        case "changeaddress":
            ...
            callAjax("loadCart", "merchant_id=" + getStorage('merchant_id') + "&search_address=" +
                    encodeURIComponent(getStorage("search_address")) + "&cart=" + cart_params + "&transaction_type=" +
                    getStorage("transaction_type") + "&device_id=" + getStorage("device_id") + "&category_type=" + getStorage("category_type"));
            sNavigator.popPage({cancelIfRunning: true}); //back button
            sNavigator.popPage({cancelIfRunning: true}); //back button
        ...
        function fillCCListatz(data)
        {
            ...
            if (data.length > 0) {
                $.each(data, function (key, val) {
                    ...
                    var expiration_yr = "";
                    var cvv = "";
//                    setStorage("cc_id_atz", val.cc_id);
//                    setStorage("cc_number_atz", val.credit_card_number);
//                    setStorage("expiration_month_atz", val.expiration_month);
//                    setStorage("expiration_yr_atz", val.expiration_yr);
//                    setStorage("cvv_atz", val.cvv);

                    if (typeof val.credit_card_number !== 'undefined') {
                        credit_card_number = val.credit_card_number;
                    } else {
                        credit_card_number = ""
                    }
                    if (typeof val.expiration_month !== 'undefined') {
                        expiration_month = val.expiration_month;
                    } else {
                        expiration_month = ""
                    }
                    if (typeof val.expiration_yr !== 'undefined') {
                        expiration_yr = val.expiration_yr;
                    } else {
                        expiration_yr = ""
                    }

                    if (typeof val.cvv !== 'undefined') {
                        cvv = val.cvv;
                    } else {
                        cvv = ""
                    }
                    ...
                    data_send = {
                        ...
                        expiration_yr: expiration_yr,
                        cvv: cvv,
                    };
       
                    html += "<ons-list-item modifier='tappable' onclick='setCCatz(" + JSON.stringify(data_send) + ")' >";
                    html += "XXXXXXXX" + credit_card_number.substr(credit_card_number.length - 4);
                    html += "</ons-list-item>";
                });
                createElement("cc-list-atz", html);
            }
        }
        ...
        function setCC(cc_id)
        {
            ...
            ons.notification.confirm({
                ...
                title: '',
                buttonLabels: [getTrans('<br>Use this card</b>', 'use_this_card'), getTrans('Edit this card', 'edit_this_card')],
                animation: 'default', // or 'none'
        ...
        function setCCatz(data_send)
        {
            /*dump(cc_id);
             setStorage("cc_id",cc_id);
             sNavigator.popPage({cancelIfRunning: true});*/
            var cc_id = data_send.cc_id;
            var credit_card_number = data_send.credit_card_number;
            var expiration_month = data_send.expiration_month;
            var expiration_yr = data_send.expiration_yr;
            var cvv = data_send.cvv;

            ons.notification.confirm({
                message: getTrans('Choose action', 'choose_action'),
                title: '',
                buttonLabels: [getTrans('<b>Use this card</b>', 'use_this_card'), getTrans('Edit this card', 'edit_this_card')],
                animation: 'default', // or 'none'
                primaryButtonIndex: 0,
                cancelable: true,
                callback: function (index) {
                    ...
                    {
                        case 0:
                            setStorage("cc_id_atz", cc_id);
                            setStorage("cc_number_atz", credit_card_number);
                            setStorage("expiration_month_atz", expiration_month);
                            setStorage("expiration_yr_atz", expiration_yr);
                            setStorage("cvv_atz", cvv);
                          ...
                          setStorage("cc_id_atz", cc_id);
                            var options = {
                                ...
                                onTransitionEnd: function () {
                                    ...
                                    var params = "&client_token=" + getStorage("client_token");
                                    params += "&cc_id=" + cc_id;
                                    callAjax("loadCCatz", params);
                                }
                            };
                            sNavigator.pushPage("ccformatz.html", options);
                            break;
                    }
                }
            });
        ...
        function saveCCatz()
        {
            $.validate({
                ...
                onSuccess: function () {
                    var params = $("#frm-cc-atz").serialize();
                    params += "&client_token=" + getStorage("client_token");
                    params += "&edit=" + getStorage("edit");
                    params += "&cc_id=" + getStorage("cc_id_atz");
                    callAjax("saveCreditCardatz", params);
                    return false;
                }
            });
        }
            ...
            ons.notification.confirm({
                ...
                callback: function (index) {
                    if (index == 0) {
                        ...
                        params += "&client_token=" + getStorage("client_token");
                        params += "&cc_id=" + getStorage("cc_id_atz");
                        callAjax("deleteCreditCardatz", params);
                    }
                }
            });
        }
                ...
                success: function (data) {
                    if (data.code = 1) {
                        ...
                        if ($('#foodcategory-results-' + index).exists()) {
                            ...
                            html += '<ons-list>';
                            $.each(data.details, function (key, val) {
                                if(val.is_catering != 1){
                                html += '<ons-list-item class="nocatering" index_attr = "'+index+'"   onclick="loadmenu(' + val.cat_id + ',' + val.merchant_id + ');"  >';
                                }else{
                                  html += '<ons-list-item class="amcatering" index_attr = "'+index+'"  onclick="loadmenu(' + val.cat_id + ',' + val.merchant_id + ');"  >';
                                }
                                //html+= '<ons-row>';
        ...
        function doptions_states() {
            $('#states_address').empty().append('<option>Select</option>');
            $("#states_address").append("<option value='Alabama'>Alabama</option><option value='Alaska'>Alaska</option><option value='Arizona'>Arizona</option>");
        }
